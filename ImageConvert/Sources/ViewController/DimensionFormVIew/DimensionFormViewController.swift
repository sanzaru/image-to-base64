...
protocol DimensionFormViewDelegate: AnyObject {
...
class DimensionFormViewController: NSViewController {
    ...
    override func viewWillAppear() {
        textfieldWidth.stringValue = String(Int(dimensions.width))
        textfieldHeight.stringValue = String(Int(dimensions.height))
    ...
    private func saveDimensions(_ sender: NSButton? = nil) {
        ...
        guard let width = Int(textfieldWidth.stringValue) else {
            showError(message: "Invalid value for width: Must be numeric.")
            return
        }

        guard let height = Int(textfieldHeight.stringValue) else {
        ...
        if textfieldHeight.stringValue.isEmpty || textfieldWidth.stringValue.isEmpty {
            ...
            if width <= 0 || height <= 0 {
                ...
                delegate?.dimensionFormViewSave(size: CGSize(width: width, height: height))
                dismiss(sender)
            }
        }
    }
